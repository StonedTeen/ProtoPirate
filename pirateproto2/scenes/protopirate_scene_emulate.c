        ...
        case ProtoPirateCustomEventEmulateTransmit:
            if (emulate_context && emulate_context->transmitter)
            {
                ...
                if (status == SubGhzProtocolStatusOk)
                {
                    ...
                    uint32_t frequency = subghz_setting_get_default_frequency(app->setting);
                    ...
                    // Use the SubGhz worker to transmit
                    subghz_worker_start(
                        app->txrx->worker,
                        (SubGhzWorkerCallback)subghz_transmitter_yield,
                        emulate_context->transmitter,
                        subghz_devices_async_tx_callback,
                ...
                {
                    FURI_LOG_E(TAG, "Failed to deserialize transmitter data: %d", status);
            ...
            else
            {
                ...
                FURI_LOG_W(TAG, "No transmitter available for this protocol");
                notification_message(app->notifications, &sequence_error);
            }
            consumed = true;
            break;
